...
command="Xvnc ${DISPLAY} -geometry ${DISPLAY_WIDTH}x${DISPLAY_HEIGHT} -depth ${DISPLAY_DEPTH} -FrameRate=${VNC_FRAMERATE} ${TIGER_VNC_OPTS} ${OTHER_ARGS}"
if [ "${DISPLAY_MAX}" = "1" ]; then
  command="Xvnc ${DISPLAY} -geometry ${DISPLAY_WIDTH_MAX}x${DISPLAY_HEIGHT_MAX} -depth ${DISPLAY_DEPTH} -FrameRate=${VNC_FRAMERATE} ${TIGER_VNC_OPTS} ${OTHER_ARGS}"
elif [ "${DISPLAY_MAX}" = "-1" ]; then
  command="Xvnc ${DISPLAY} -depth ${DISPLAY_DEPTH} -FrameRate=${VNC_FRAMERATE} ${TIGER_VNC_OPTS} ${OTHER_ARGS}"
fi
